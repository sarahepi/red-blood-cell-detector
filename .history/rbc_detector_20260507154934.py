...
"""
    ...
    def watershed(
        ...
        Étapes :
        1. sure_bg  = masque morphologique dilaté
                        (arrière-plan certain)

        2. sure_fg  = carte de distance seuillée
                        (centres des cellules certains)

        3. unknown  = sure_bg - sure_fg
                        (pixels frontières incertains)

        4. Étiqueter les composantes connexes dans sure_fg
            afin de créer les marqueurs

        5. Marquer la région unknown avec 0
            (laisser Watershed décider)

        6. Appliquer cv2.watershed
            → les frontières détectées sont marquées par -1
        """
    ...
    def detect_contours(
        ...
        Sans Watershed :
        les contours sont extraits directement à partir du masque morphologique.

        Avec Watershed :
        chaque étiquette unique (>1) correspond à une cellule distincte ;
        chaque région étiquetée est re-binarisée afin d’en extraire le contour.

        Critères de filtrage (configurables) :
        • Aire : élimine les poussières (trop petites)
                    et les groupes de cellules (trop grands)

        • Circularité : 4π·Aire / Périmètre²
                        → les globules rouges sont approximativement circulaires
        """
    ...
    def _is_valid(self, contour: np.ndarray) -> bool:
        """
        Rejette les contours qui ne correspondent clairement pas à des globules rouges (RBC) individuels.

        Circularité = 4π·A / P²
        = 1.0  → cercle parfait
        ≈ 0.75 → légèrement allongé
        < 0.5  → forme très irrégulière (probablement du bruit ou deux cellules fusionnées)
        """
