...
class RBCApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.geometry(f"{min(1380, sw)}x{min(820, sh-40)}")
        self.state("zoomed") if sw >= 1200 else None   # maximise on large screens
    ...
    def _build_ui(self):
        ...
        # Left panel (controls) — scrollable so buttons are always reachable
        left_outer = tk.Frame(main, bg=PANEL, width=300)
        left_outer.pack(side="left", fill="y", padx=(0, 8))
        left_outer.pack_propagate(False)

        # Canvas + scrollbar for vertical scrolling
        left_canvas = tk.Canvas(left_outer, bg=PANEL, highlightthickness=0, width=290)
