...
Pipeline overview:
  1. Load image
  2. Resize (normalisation géométrique)        ← NOUVEAU
  3. ROI masking (région d'intérêt)            ← NOUVEAU
  4. Convert to grayscale
  5. Gaussian blur  → noise reduction
  6. Canny edge detection (visualisation)      ← NOUVEAU
...
class PipelineResult:
    ...
    resized:       Optional[np.ndarray] = None   # ← NOUVEAU : après resize
    roi_mask:      Optional[np.ndarray] = None   # ← NOUVEAU : masque ROI
    roi_applied:   Optional[np.ndarray] = None   # ← NOUVEAU : image après ROI
    gray:          Optional[np.ndarray] = None
    blurred:       Optional[np.ndarray] = None
    edges:         Optional[np.ndarray] = None   # ← NOUVEAU : Canny edges
